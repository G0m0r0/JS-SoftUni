function solve() {
    ...
    $elements={
       ...
       InProgressSpace: document.getElementsByTagName('section')[2],
    ...
    function finishButton(e){
        ...
        //$elements.InProgressSpace.innerHTML='';
        e.target.parentNode.parentNode.remove();
    }

    function startButton(e){
        let divToAttachAll=$elements.InProgressSpace.getElementsByTagName('div')[1];
        let task=e.target.parentNode.parentNode.getElementsByTagName('h3')[0].textContent;
        let description=e.target.parentNode.parentNode.getElementsByTagName('p')[0].textContent.split(':')[1].trim();
        let date=e.target.parentNode.parentNode.getElementsByTagName('p')[1].textContent.split(':')[1].trim();
        ...
        let h3=createHtmlElement('h3',null,task);
        let p1=createHtmlElement('p',null,`Description: ${description}`);
        let p2=createHtmlElement('p',null,`Due Date: ${date}`);
        let article=createHtmlElement('article');
